...
author: "Guest Data Unit (Assimilation Department)"
...
#                         col_names = TRUE) %>%
 ...
 guest_data <- read_excel("Data Captured - Sun 7th August, 2022 - Gbagada.xlsx",
                         sheet = "All Data",
                          col_names = TRUE) %>% 
...
## Guest analysis by Service Mode
...
guest_only %>% 
  ...
  ggplot(mapping = aes(x = 2, y = prop, fill = `Service Mode`))+
...
guest_only %>% 
  ...
  ggplot(mapping = aes(x = 2, y = prop, fill = `Age Range`))+
  geom_bar(width = 1, color = "white", stat = "identity") +
  xlim(0.5, 2.5) +
  coord_polar(theta = "y", start = 0) +
  theme_void() +
...
<!-- * We recorded a total guest ($1^{st}$ and $2^{nd}$ timers only) count of `r dim(guest_only)[1]`. with an average age of `r round(median(guest_only$Age, na.rm = TRUE),0)` years. -->
...
guest_only %>% 
  ...
  ggplot(mapping = aes(x = reorder(`My decision today`, prop), y = prop))+
  geom_bar(width = 0.7, fill = "steelblue", stat = "identity") +
  scale_y_continuous(labels = percent) +
  geom_text(aes(label = paste0(round(prop*100, 1), "%")), size = 3, position = position_stack(vjust = 0.5)) +
  scale_fill_brewer(palette = "Blues", direction = 1) +
  #theme(axis.text.x = element_text(angle = 90), legend.position = "top")+
...
guest_only %>% 
  ...
  ggplot(mapping = aes(x = reorder(District, prop), y = prop))+
  geom_bar(width = 0.7, fill = "steelblue", stat = "identity") +
  scale_y_continuous(labels = percent) +
  geom_text(aes(label = paste0(round(prop*100, 1), "%")), size = 3, position = position_stack(vjust = 0.5)) +
  scale_fill_brewer(palette = "Blues", direction = 1) +
...
-   For feedback, kindly reach the Head of Guest Data Unit via ([**a.dayoajimuda\@gmail.com**](mailto:a.dayoajimuda@gmail.com){.email})
